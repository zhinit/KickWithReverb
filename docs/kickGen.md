...
This model is integrated into the kick-with-reverb app via the Django backend, which exposes a generation API endpoint to the React frontend.

## Directory Structure

All model code and weights live in `pytorch/` at the project root:
...
`scipy` is only used for writing WAV files (`scipy.io.wavfile.write`). If the integration handles the waveform tensor directly (e.g. returning audio bytes via the API), scipy can be skipped.
...
from pytorch.inference.generate import generate

output_path = generate(
    diffusion_checkpoint=Path("pytorch/checkpoints/diffusion_step_100000.pt"),
    vae_checkpoint=Path("pytorch/checkpoints/vae_epoch_100.pt"),
    vocoder_checkpoint=Path("pytorch/checkpoints/vocoder_epoch_50.pt"),
    prompt="808",
    cfg_scale=3.0,
    ddim_steps=50,
...
If you need the raw tensor instead of a file (e.g. for returning audio bytes from the API), you can replicate the pipeline steps from `generate()`:

```python
import torch
from pytorch.models.autoencoder import KickVAE
from pytorch.models.diffusion import LatentUNet, NoiseScheduler
from pytorch.models.text_encoder import KeywordEncoder
from pytorch.models.vocoder import HiFiGANGenerator
from pytorch.inference.generate import DDIMSampler, parse_prompt

device = torch.device("cpu")

# Load diffusion model
diff_ckpt = torch.load("pytorch/checkpoints/diffusion_step_100000.pt", weights_only=False, map_location=device)
...
# Decode with VAE
vae_ckpt = torch.load("pytorch/checkpoints/vae_epoch_100.pt", weights_only=False, map_location=device)
...
voc_ckpt = torch.load("pytorch/checkpoints/vocoder_epoch_50.pt", weights_only=False, map_location=device)
...
```python
ckpt = torch.load("pytorch/checkpoints/diffusion_step_100000.pt", weights_only=False)
