...
```bash
cd dsp && emcmake cmake -B build && cmake --build build
```

Output: `react/public/audio-engine.js` (SINGLE_FILE with embedded WASM)
...
4. Reverb (send): (kick+noise) → StereoConvolutionReverb(wet=1.0) → LowpassFilter → HighpassFilter → reverbGain_
   (skipped if no IR loaded, outputs silence)
...
### Reverb
- `loadIR(ptr, irLength, numChannels)` — Store IR in irStorage_ vector
...
### Transport
- `setBPM(bpm)` — Recalculates samplesPerBeat_ (60-365)
...
- **IR storage**: Raw IR data stored in `irStorage_` vector. `selectIR` reloads the convolution engine (not instant, involves FFT).
