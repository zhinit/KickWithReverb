...
void AudioEngine::process(uintptr_t leftPtr, uintptr_t rightPtr, int numSamples)
{
...
void AudioEngine::loadKickSample(uintptr_t ptr, size_t length)
{
    kickPlayer_.loadSample(ptr, length);
}

void AudioEngine::selectKickSample(int index)
{
    kickPlayer_.selectSample(index);
}

void AudioEngine::setKickRelease(float seconds)
{
    kickPlayer_.setReleaseDuration(seconds);
}

void AudioEngine::setKickDistortion(float amount)
{
    kickDistortionMix_ = std::clamp(amount, 0.0f, 1.0f);
}

void AudioEngine::setKickOTT(float amount)
{
...
void AudioEngine::loadNoiseSample(uintptr_t ptr, size_t length)
{
    noisePlayer_.loadSample(ptr, length);
}

void AudioEngine::selectNoiseSample(int index)
{
    noisePlayer_.selectSample(index);
}

void AudioEngine::setNoiseVolume(float db)
{
    noisePlayer_.setVolume(std::pow(10.0f, db / 20.0f));
}

void AudioEngine::setNoiseLowPass(float hz)
{
    noiseLowPass_.setFrequency(hz);
}

void AudioEngine::setNoiseHighPass(float hz)
{
...
void AudioEngine::loadIR(uintptr_t ptr, size_t irLength, size_t numChannels)
{
...
void AudioEngine::selectIR(int index)
{
...
void AudioEngine::setReverbLowPass(float hz)
{
    reverbLowPass_.setFrequency(hz);
}

void AudioEngine::setReverbHighPass(float hz)
{
    reverbHighPass_.setFrequency(hz);
}

void AudioEngine::setReverbVolume(float db)
{
...
void AudioEngine::setMasterOTT(float amount)
{
    masterOTT_.setAmount(std::clamp(amount, 0.0f, 1.0f));
}

void AudioEngine::setMasterDistortion(float amount)
{
    masterDistortionMix_ = std::clamp(amount, 0.0f, 1.0f);
}

void AudioEngine::setMasterLimiter(float amount)
{
...
void AudioEngine::cue()
{
    kickPlayer_.trigger();
}

void AudioEngine::recalcSamplesPerBeat()
{
    if (bpm_ > 0.0f) {
        samplesPerBeat_ =
            static_cast<int>(sampleRate_ * 60.0f / bpm_);
    }
}
